...
class _ViolationsState extends State<Violations> {
  Future<List<Map<String, dynamic>>> fetchPatrollingHistory() async {
    try {
      ...
      final uri = Uri.parse(
          "https://api1.aksasoft.net/patr_dplganderbal/webservices/view_trails.php?id=${androidId.toString()}&lat=${position.latitude}&long1=${position.longitude}&token=${boxx.get("token")}&passcode=97786068765874&fingerprint=biometric123&mobile=${boxx.get("number")}&accuracy=${position.accuracy}&violation=location");
